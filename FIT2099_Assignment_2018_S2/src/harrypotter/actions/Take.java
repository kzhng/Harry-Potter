 ...
 */
/*
 * Changelog 2017/01/26 - candDo method changed. An actor can only take if it's
 * not holding any items already. - act method modified. Take affordance removed
 * from the item picked up, since an item picked up cannot be taken. This is
...
public class Take extends HPAffordance {
	...
	/**
	 * Returns if or not this <code>Take</code> can be performed by the
	 * <code>HPActor a</code>.
	 * <p>
	 * This method returns true if and only if <code>a</code> is not carrying any
	 * item already.
	 * 
	 * @author ram
	 ...
	 * <code>HPActor</code> to the target ( the <code>HPActor a</code>'s item
	 * carried would be the target of this <code>Take</code>).
	 * <p>
	...
	public void act(HPActor a) {
		if (target instanceof HPEntityInterface) {
			...
			target.addAffordance(new Leave(theItem, messageRenderer)); // kzhe added leave affordance to the item so
																		// that you can repeatedly leave
			// and take an item
		}
	}
	 ...
	 * 
	 * @author ram
	 * @return String comprising "take " and the short description of the target of
	 *         this <code>Take</code>
	 */
	@Override
